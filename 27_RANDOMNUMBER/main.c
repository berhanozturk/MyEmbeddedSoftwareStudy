...
uint32_t numberArray[]={0x3F,0x6,0x5B,0x4F,0x66,0x6D,0x7D,0x7,0x7F,0x6F};
...
void randomNumber(GPIO_TypeDef* port){
		uint32_t random=rand()%9;
		char rLetter='A'+rand()%7;
		printNumber(port,random);
		delay(3600000);
		printLetters(port,rLetter);
		delay(3600000);
}
int main(){
